...
To evaluate, I'm pleasantly content with how the app has turned out, especially since I was able to incorporate the course content learnt thus far into one project. Having a design stage before producing the actual application really helps with giving you a rough idea of how the app should look/function so that's definitely something that worked well with this project - Figma itself is an incredibly useful tool for this.

I view this as being a success since it meets all of my functional requirements, everything works as intended e.g.the input validation is able to catch invalid inputs correctly and block submission attempts. The CSV is also in a reliable state, recording the intended information which can then be accessed using standard software. As an improvement, I think it would be a good idea to add score calculation within the app, definitely something that would be possible with the MCQ format. At present, responses have to be reviewed manually through the CSV, but it would be much more efficient if the app could automatically compare selected answers against the correct answers in the CSV and produce a score that way. This would save time for staff and make the tool more useful.

Something which caused a few hiccups for me quite often in this project was indentations. Often, my code would fail and I would have no reason why...only to realise later on that it's related to the indentations - undoubtedly something I should pay more mind to!

I felt as though the GUI design colours in the prototype stage were a bit off, even though they followed the brand imagery, the bright red background was moreso a distraction after a few minutes and wasn't easy on the eye, so I ended up changing this when actually making the GUI. The current design is better in my opinion, it still follows brand colours, whilst creating a better viewing experience. Noticeably, the brand logo is also not present in the final app - perhaps this is something that I could add as an improvement, I could see the benefit in that it would assist in identifying this app as an official resource. There's certainly room for development with the GUI as a whole. Whilst the current design is functional and relates to the brand, it could use some polish, such as better spacing or even something like a progress bar to improve user experience. Perhaps a picture of a train in the background would also be a neat addition😊.
